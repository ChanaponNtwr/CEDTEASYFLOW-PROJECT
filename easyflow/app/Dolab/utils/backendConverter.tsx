...
      draggable: false,
      sourcePosition: "bottom" as any, // <-- แก้ตรงนี้
      targetPosition: "top" as any,    // <-- และตรงนี้
    };
...
  // --- constants ---
  const BREAKPOINT_CHILD_SHIFT = 73; // This is now legacy, but kept for reference
...
      if (trueChild) {
        // Use finalX as the base for child calculations
...
      
      ...
      // This ensures children of breakpoints (and all other nodes) are positioned directly below them.
...
  // applyEdgeHandles now uses outgoingBySource + edgeId and incomingByTarget to determine correct index/handle
  const applyEdgeHandles = (edge: Edge, srcNode?: Node, tgtNode?: Node, conditionRaw?: string) => {
    ...
    const setSource = (handle: string, offset = 0) => {
      ...
      // ใช้ as any เมื่อเขียน pathOptions
