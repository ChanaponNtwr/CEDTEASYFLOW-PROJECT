...
function restoreContextFromSnapshot(ctx, snapshot) {
  ...
  if (Array.isArray(scopeStack) && scopeStack.length > 0) {
    ...
    ctx.output = Array.isArray(snapshot.output)
      ? Array.from(snapshot.output)
      : [];
    return;
  }
...
function applyUserVariables(ctx, vars) {
  ...
  for (const v of vars) {
    ...
    try {
      ...
      console.warn(
        `applyUserVariables: failed to set '${v.name}':`,
        e?.message,
      );
    }
  }
...
router.post("/start", async (req, res) => {
  try {
    ...
    if (!labId)
      return res.status(400).json({ ok: false, error: "Missing labId" });
    ...
    if (!state)
      return res
        ...
        .json({
          ok: false,
          error:
            "Trial session not found or expired. Please start a new trial.",
        });
...
router.post("/:trialId/flowchart/node", async (req, res) => {
  try {
    ...
    if (!edgeId)
      return res.status(400).json({ ok: false, error: "Missing edgeId" });
    ...
    if (!fc.getEdge(edgeId))
      return res
        .status(400)
        .json({ ok: false, error: `Edge '${edgeId}' not found` });

    const shapeRemainingBefore = computeUsageAndRemaining(
      state.flowchart,
      state.labRow,
    );
    ...
    // ── shape limit check ──
    const info = shapeRemainingBefore[incomingType] || {
      limit: "unlimited",
      remaining: "unlimited",
    };
    if (info.limit !== "unlimited" && Number(info.remaining) <= 0) {
      return res
        ...
        .json({
          ...
          shapeRemaining: shapeRemainingBefore,
        });
    }
    ...
    if (incomingType === "DC" && !Boolean(force)) {
      ...
      if (typeof d.name === "string" && d.name.trim())
        newNames.push(d.name.trim().toLowerCase());
      if (typeof d.varName === "string" && d.varName.trim())
        newNames.push(d.varName.trim().toLowerCase());
      if (Array.isArray(d.names)) {
        for (const nm of d.names) {
          if (typeof nm === "string" && nm.trim())
            newNames.push(nm.trim().toLowerCase());
        }
      }
...
router.put("/:trialId/flowchart/node/:nodeId", async (req, res) => {
  try {
    ...
    if (!node)
      return res
        .status(404)
        .json({ ok: false, error: `Node '${nodeId}' not found` });
...
router.post("/:trialId/execute", async (req, res) => {
  try {
    ...
    if (action === "step") {
      ...
      try {
        contextSnapshot =
          typeof executor.context.serialize === "function"
...
router.post("/:trialId/testcases/run", async (req, res) => {
  try {
    ...
    if (!testcases.length)
      return res
        .status(400)
        .json({ ok: false, error: "No testcases available for this lab" });
    ...
    for (const tc of testcases) {
      ...
      try {
        ...
        const passed =
          JSON.stringify(actualOutput) === JSON.stringify(expectedVal);
