...
class Executor {
  ...
  setInputProvider(fn) {
    if (typeof fn === "function") {
      ...
      if (this.flowchart && this.flowchart._inputProvider)
  ...
  serializeState() {
    try {
      ...
      if (this.flowchart && this.flowchart.nodes) {
        for (const [nid, nodeObj] of Object.entries(this.flowchart.nodes)) {
          nodesState[nid] = {
            ...
            _initValue:
              typeof nodeObj._initValue !== "undefined"
                ? nodeObj._initValue
                : null,
          };
        }
      ...
      if (Array.isArray(this.context._scopeStack)) {
        try {
          ctx._scopeStack = JSON.parse(
            JSON.stringify(this.context._scopeStack),
          );
        } catch (e) {
    ...
    try {
      ...
      if (state.context && typeof state.context === "object") {
        try {
          ...
          if (
            Array.isArray(state.context._scopeStack) &&
            Array.isArray(this.context._scopeStack)
          ) {
            this.context._scopeStack = JSON.parse(
              JSON.stringify(state.context._scopeStack),
            );
            if (typeof this.context._syncVariables === "function")
              this.context._syncVariables();
              ...
              if (v && v.name) {
                try {
                  ...
                  try {
                    ...
                    const existingIndex = this.context.variables.findIndex(
                      (x) => x.name === v.name,
                    );
                    if (existingIndex !== -1)
                      this.context.variables[existingIndex] = {
                        name: v.name,
                        value: v.value,
                        varType: v.varType,
                      };
                    ...
                    if (typeof this.context._rebuildIndexMap === "function")
                      this.context._rebuildIndexMap();
                  } catch (ee) {
                    /* ignore */
                  }
                }
          ...
          if (Array.isArray(state.context.output))
            this.context.output = Array.from(state.context.output);
        } catch (e) {
          console.warn(
            "Executor.restoreState: context restore partial failure:",
            e,
          );
        }
      }
      ...
      if (state.nodesState && this.flowchart && this.flowchart.nodes) {
        for (const [nid, snapshot] of Object.entries(state.nodesState)) {
          ...
          if ("_initialized" in snapshot)
            node._initialized = Boolean(snapshot._initialized);
          if ("_phase" in snapshot) node._phase = snapshot._phase;
          if ("_loopCount" in snapshot)
            node._loopCount = Number(snapshot._loopCount || 0);
          if ("_scopePushed" in snapshot)
            node._scopePushed = Boolean(snapshot._scopePushed);
  ...
  step(stepOptions = {}) {
    ...
    if (elapsed > this.maxTimeMs) {
      const err = new Error(
        `Execution time limit exceeded (${elapsed}ms > ${this.maxTimeMs}ms)`,
      );
    ...
    if (this.stepCount >= this.maxSteps) {
      const err = new Error(
        `Max step count exceeded (${this.stepCount} >= ${this.maxSteps})`,
      );
      console.error(err.message);
      this.finished = true;
      return { error: err, done: true, context: this.context };
    ...
    if (this.paused) {
      if (!this._pendingNextEdgeId) {
        console.warn(
          "Executor.step: paused but no pending edge — clearing pause",
        );
        this.paused = false;
      ...
      if (!pendingEdge) {
        console.error(
          `Pending edge ${this._pendingNextEdgeId} not found when resuming from breakpoint`,
        );
        this.paused = false;
        this._pendingNextEdgeId = null;
        return {
          error: new Error("Pending edge not found"),
          done: this.finished,
    ...
    console.log(
      `Step ${this.stepCount + 1}: Executing node ${node.id} (${node.type})`,
    );

    // Ensure the effective inputProvider is applied to flowchart for handlers
    const effectiveProvider =
      typeof handlerOptions.inputProvider === "function"
    ...
    try {
      result =
        handler(node, this.context, this.flowchart, handlerOptions) || {};
    } catch (err) {
      console.error(
        `Error in handler for node ${node.id} (${node.type}):`,
    ...
    if (!wantsReenter) {
      if (["WH", "FR"].includes(node.type)) {
        nextEdgeId =
          result.nextNode || node.loopExitEdge || node.outgoingEdgeIds[0];
        ...
        if (result.nextCondition) {
          const edge = outgoingEdges.find(
            (e) => e.condition === result.nextCondition,
          );
    ...
    if (node.type === "BP" && !forceAdvanceBP) {
      ...
      this.paused = true;
      console.log(
        `Hit breakpoint at ${node.id}; execution paused. Call step() or resume() to continue.`,
      );
    ...
    if (nextEdgeId) {
      ...
      if (!nextEdge) {
        console.error(`Edge ${nextEdgeId} not found`);
        this.finished = true;
      } else {
      ...
      console.log(" No next edge, stopping execution");
      this.finished = true;
    }
  ...
  runAll(options = {}) {
    ...
    while (!this.finished) {
      const res = this.step({
        forceAdvanceBP: ignoreBreakpoints,
        ...handlerOptions,
      });
