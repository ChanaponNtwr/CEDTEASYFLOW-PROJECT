...
const NodeTypeShort = Object.freeze(
  Object.fromEntries(
    Object.entries(NodeType).map(([short, full]) => [full, short]),
  ),
);
...
class Node {
  constructor(
    ...
    if (NodeTypeShort[type]) shortType = NodeTypeShort[type];
      ...
      if (!Object.keys(NodeType).includes(type))
    ...
    if (missingKeys.length > 0) {
      throw new Error(
        `Node ${id} type ${shortType} ขาด data keys: ${missingKeys.join(", ")}`,
      );
    }
  ...
  static getDefaultData(shortType) {
    switch (shortType) {
      ...
      case "EN":
        return { label: "End" };
      case "DC":
        return { name: "", value: null, varType: "" };
      case "IF":
        return { condition: "" };
      case "AS":
        return { variable: "", value: "" };
      case "FR":
        return { init: "", condition: "", increment: "", varName: "" };
      case "WH":
        return { condition: "", varName: "", increment: "" };
      case "IN":
        return { variable: "", prompt: "", varType: "" };
      case "OU":
        return { message: "" };
      case "BP":
        return { note: "" };
  ...
  updateData(newData) {
    ...
    if (missing.length > 0)
      throw new Error(
        `Node ${this.id} type ${this.type} ขาด data keys: ${missing.join(", ")}`,
      );
    ...
    try {
      ...
      switch (this.type) {
        ...
        case "DC": // declare: e.g. "int x = 0"
          this.label =
            `${d.varType || ""} ${d.name || ""}${d.value !== undefined && d.value !== null ? ` = ${d.value}` : ""}`.trim();
          break;
        case "AS": // assign: "x = 5"
        ...
        case "FR":
          this.label =
            `for (${d.init || ""}; ${d.condition || ""}; ${d.increment || ""})`.trim();
          break;
        case "WH":
          this.label = `while (${d.condition || ""})`.trim();
          break;
        case "IN":
          this.label = `input ${(d.variable || "").toString().trim()}`;
          break;
        case "OU":
