...
export function hydrateFlowchart(payload = {}) {
  ...
  const nodesInput = Array.isArray(payload.nodes)
    ? payload.nodes
    : payload.nodes
  ...
  try {
    const hasStartOutgoingOtherThanDefault = Object.values(fc.edges).some(
      (e) => e.source === "n_start" && e.target !== "n_end",
    );
  ...
  const meaningfulEdges = Object.keys(fc.edges).filter(
    (id) => id !== "n_start-n_end",
  );
  if (nodesInput.length > 0 && meaningfulEdges.length === 0) {
    // insert sequentially into start->end (best-effort)
    let currentEdgeId = fc.chooseOutgoingEdgeId("n_start", [
      "auto",
      "next",
      "true",
      "false",
    ...
    for (const n of nodesInput) {
      ...
      const outEdgeId = (newNode.outgoingEdgeIds || []).find((id) =>
        fc.getEdge(id),
      );
      if (!outEdgeId) break;
