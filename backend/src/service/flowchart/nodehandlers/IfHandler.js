...
export default function IfHandler(node, context) {
  try {
    // build variable names and values from context
    const keys = (context.variables || [])
      .map((v) => v && v.name)
      .filter(Boolean);
    ...
    const conditionRaw =
      node && node.data && node.data.condition
    ...
    console.error(
      `Error evaluating If condition '${(node && node.data && node.data.condition) || ""}': ${e && e.message ? e.message : e}`,
    );
    return { nextCondition: "false" };
  }
}
