...
export default class TestRunner {
  ...
  cloneFlowchart(flowchart) {
    ...
    try {
      ...
      // Preserve explicit start node if present at top-level
      if (flowchart.startNodeId) plainData.startNodeId = flowchart.startNodeId;
      if (flowchart.start) plainData.start = flowchart.start;

      // 2. Sanitize: Clear runtime data only (do NOT remove node-level logic)
      if (plainData.nodes) {
        ...
        nodes.forEach(node => {
          if (node.data) {
             // remove ephemeral runtime flags/counters — do not remove logic values.
      ...
      // 3. Re-hydrate
      const hydrated = hydrateFlowchart(plainData);

      // if hydrateFlowchart returns a plain object missing startNodeId, ensure it's set
      if (hydrated && (hydrated.startNodeId === undefined) && (plainData.startNodeId || plainData.start)) {
        hydrated.startNodeId = plainData.startNodeId || plainData.start;
      }
  ...
  async runBatch(flowchart, testcases = [], userId = null) {
    ...
    for (const tc of tcInstances) {
      ...
      try {
        ...
        // parse inputs to numbers where possible
      ...
      try {
        ...
        // Manual Step Loop (matches controller)
        try {
